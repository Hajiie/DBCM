        ...
        <Link href="/" passHref>
          {
            logo ?
              logo.type === LogoType.IMAGE ?
                <img src={theme === THEMES.DARK ? transformImagePaths(logo.logoLight) : transformImagePaths(logo.logo)} alt="WebExpe" className="cursor-pointer" width="50px" /> :
        ...
        <div className='text-[14px] font-normal items-center lg:flex hidden'>
          {
            navLinks.map((each: iNavLink, i: any) => (
              each.type !== 'dropdown' ? !each.newTab ?
                ...
                :
                <NavCatergoryDD key={i} label={each.label} openDD={openDD} setOpenDD={() => setOpenDD(!openDD)} floating />
            ))
          }
          {
            socials &&
